...
SLOT="0/2-sdl2"
KEYWORDS="~amd64 ~x86"
IUSE="debugger gles2-only lirc +new-dynarec2 opencv +osd cpu_flags_x86_sse"
...
src_prepare() {
	...
	# fix building against >=libbfd-2.39
	has_version '>=sys-libs/binutils-libs-2.39' &&
		eapply "${FILESDIR}"/${P}-debugger-fix-binutils-2.39-compat.patch
}
...
src_compile() {
	MAKEARGS=(
		...
		NEW_DYNAREC=$(usex new-dynarec2 1 0)
