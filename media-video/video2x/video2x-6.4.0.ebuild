...
PATCHES=(
	"${FILESDIR}/${PN}-0001-build-system-fix.patch"
	"${FILESDIR}/${PN}-0002-arm64-target-clones.patch"
)
...
src_prepare() {
	...
	sed -re "s/(LIBRARY|ARCHIVE) DESTINATION lib/\1 DESTINATION $(get_libdir)/g" \
		-i third_party/libr*/src/CMakeLists.txt || die
