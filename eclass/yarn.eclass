...
if [[ ! ${_YARN_ECLASS} ]]; then
    ...
    yarn_uris() {
        local -r regex='^(@[a-zA-Z0-9_-]+/)?([a-zA-Z0-9_-]+)-([0-9]+\.[0-9]+\.[0-9]+.*)'
        if [[ -z ${YARN_PKGS} ]]; then
        ...
        for pkg in ${YARN_PKGS}; do
            ...
            url="https://registry.yarnpkg.com/${scope}${name}/-/${name}-${version}.tgz"
            echo "${url}"
        done
    }
    ...
    src_compile() {
        cd lib || die
        cp "${YARN_PACKAGE_JSON}" package.json || die
        cp "${YARN_LOCK}" yarn.lock || die
        env \
